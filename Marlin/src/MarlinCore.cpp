...
#include "HAL/shared/cpu_exception/exception_hook.h"
  ...
  #include "RobinWiFi_KP3/wifi_module.h"
#endif // ROBIN_WIFI
...
inline void manage_inactivity(const bool no_stepper_sleep=false) {
  ...
  if (ELAPSED(ms, next_check_axes_ms)) {
    ...
    if (ELAPSED(ms, FET_next)) {
      FET_next = ms + FET_SAFETY_DELAY;  // 2μs pulse every FET_SAFETY_DELAY mS
 ...
 *    窶｢ MAX Thermocouple pins
 *    窶｢ Duet Smart Effector
 *    窶｢ Filament Runout Sensor
 *    窶｢ TMC220x Stepper Drivers (Serial)
 *    窶｢ PSU control
 *    窶｢ Power-loss Recovery
 *    窶｢ Stepper Driver Reset: DISABLE
 *    窶｢ TMC Stepper Drivers (SPI)
 *    窶｢ Run hal.init_board() for additional pins setup
 *    窶｢ ESP WiFi
 ...
 *    窶｢ Color LED illumination
 *    窶｢ Neopixel illumination
 *    窶｢ Controller Fan
 *    窶｢ Creality DWIN LCD (show boot image)
 *    窶｢ Tare the Probe if possible
 ...
 *    窶｢ Temperatures
 *    窶｢ Print Job Timer
 *    窶｢ Endstops and Endstop Interrupts
 *    窶｢ Stepper ISR - Kind of Important!
 *    窶｢ Servos
 *    窶｢ Servo-based Probe
 *    窶｢ Photograph Pin
 *    窶｢ Laser/Spindle tool Power / PWM
 *    窶｢ Coolant Control
 *    窶｢ Bed Probe
 *    窶｢ Stepper Driver Reset: ENABLE
 *    窶｢ Digipot I2C - Stepper driver current control
 *    窶｢ Stepper DAC - Stepper driver current control
 *    窶｢ Solenoid (probe, or for other use)
 *    窶｢ Home Pin
 *    窶｢ Custom User Buttons
 *    窶｢ Red/Blue Status LEDs
 *    窶｢ Case Light
 *    窶｢ Prusa MMU filament changer
 *    窶｢ Fan Multiplexer
 *    窶｢ Mixing Extruder
 *    窶｢ BLTouch Probe
 *    窶｢ I2C Position Encoders
 *    窶｢ Custom I2C Bus handlers
 *    窶｢ Enhanced tools or extruders:
 *      窶｢ Switching Extruder
 *      窶｢ Switching Nozzle
 *      窶｢ Parking Extruder
 *      窶｢ Magnetic Parking Extruder
 *      窶｢ Switching Toolhead
 *      窶｢ Electromagnetic Switching Toolhead
 *    窶｢ Watchdog Timer - Also Kind of Important!
 *    窶｢ Closed Loop Controller
