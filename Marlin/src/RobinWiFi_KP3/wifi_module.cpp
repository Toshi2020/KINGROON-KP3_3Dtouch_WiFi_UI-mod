...
/// KP3追加(tft_lvgl_configuration.cppより移植)
uint8_t bmp_public_buf[TRANS_RCV_FIFO_BLOCK_NUM * 1024];
uint8_t public_buf[513];

/// KP3追加(draw_ui.cppより移植)
CFG_ITMES gCfgItems;
UI_CFG uiCfg;

/// KP3追加(draw_wifi_list.cppより移植)
WIFI_LIST wifi_list;

/// KP3追加(draw_print_file.cppより移植)
LIST_FILE list_file;
uint8_t sel_id = 0;

/// KP3追加(draw_printing.cppより移植)
...
#ifndef LONG_FILENAME_HOST_SUPPORT // KP3変更(WiFiで長いファイル名を使わないいなら)
...
#ifdef LONG_FILENAME_HOST_SUPPORT // KP3変更(WiFiで長いファイル名を受け取る)
    strcpy(tmp, card.longest_filename());
#else
...
#define Fstream tmp	/// KP3変更Fstreamは不要
  ...
  if (strchr((char *)cmd_line, '\n') && (strchr((char *)cmd_line, 'G') || strchr((char *)cmd_line, 'M') || strchr((char *)cmd_line, 'T'))) {
    ...
    if (tmpStr) {
      ...
      switch (cmd_value) {
        ...
        case 20: // M20: Print SD / USB disk file
          ...
          if (uiCfg.print_state == IDLE) {
            ...
            #ifdef FIXED_FILENAME	// ファイル名を固定するなら
            strcpy_P((char *)&tmpStr[index], PSTR(FIXED_FILENAME));
            #endif // FIXED_FILENAME
...
#ifdef LONG_FILENAME_HOST_SUPPORT // KP3変更(WiFiで長いファイル名を受け取る)
                strcpy((char *)list_file.file_name[sel_id], PSTR("/"));
                strcpy((char *)list_file.long_name[sel_id], PSTR("/"));
                strcat((char *)list_file.file_name[sel_id], (char *)&tmpStr[index]);
                strcat((char *)list_file.long_name[sel_id], (char *)&tmpStr[index]);
                char *cur_name = (char *)list_file.long_name[sel_id];
#else
        ...
        case 24:
          if (uiCfg.print_state == PAUSED)	/// ポーズ状態からの再開なら
            queue.enqueue_one(F("M24"));    /// Marlinの処理に任せる
          else if (strcmp_P(list_file.file_name[sel_id], PSTR("notValid")) != 0) {
            if (uiCfg.print_state == IDLE) {
///              clear_cur_ui();
///              reset_print_time();
///              start_print_time();
...
///                if (!gcode_preview_over) {
                if (true) {
                  ...
                  // ファイルがあるならタイマーとプログレスをリセット
#ifndef PRINTJOB_TIMER_AUTOSTART // 自動スタートしてるならタイマーはそのまま
...
///            print_rate = uiCfg.totalSend;
            print_rate = ui.get_progress_percent();	/// KP3変更 Marlinより
            ZERO(tempBuf);
...
///          queue.enqueue_one(F("M105"));
        ...
        case 992:
          if ((uiCfg.print_state == WORKING) || (uiCfg.print_state == PAUSED)) {
            ...
            uint32_t tim = elapsed.value % 86400;
            uint8_t h, m, s;
            h = tim / 3600;
            tim %= 3600;
            m = tim / 60;
...
///          send_to_wifi((uint8_t *)"FIRMWARE_NAME:Robin_nano\r\n", strlen("FIRMWARE_NAME:Robin_nano\r\n"));
          send_to_wifi((uint8_t *)"FIRMWARE_NAME:Robin_mini\r\n", strlen("FIRMWARE_NAME:Robin_mini\r\n"));
          break;
...
static void file_first_msg_handle(uint8_t * msg, uint16_t msgLen) {
  ...
  #ifdef FIXED_FILENAME	// ファイル名を固定するなら
  char fixedfilename[256];
  strcpy_P(fixedfilename, PSTR(FIXED_FILENAME));
  fileNameLen = strlen(fixedfilename);
  memcpy(file_writer.saveFileName, fixedfilename, fileNameLen);
  #else
...
#ifdef LONG_FILENAME_WRITE_SUPPORT	// KP3変更(WiFiで長いファイル名を許容する)
    strcpy((char *)saveFilePath, (const char *)file_writer.saveFileName);
#else
    char dosName[FILENAME_LENGTH];
...
///  lv_draw_dialog(DIALOG_TYPE_UPLOAD_FILE);
  ...
  ui.clear_lcd();
  tft.queue.reset();
  tft_string.set("WiFi file receiving...");
  tft.canvas(0, 0, TFT_WIDTH, TFT_HEIGHT);
  tft.set_background(COLOR_BACKGROUND);
  tft.add_text(tft_string.center(TFT_WIDTH), TFT_HEIGHT / 2,
  										COLOR_STATUS_MESSAGE, tft_string);
  tft.queue.sync();
///  lv_task_handler();
///  get_wifi_commands();
...
void mks_wifi_firmware_update() {
  ...
  if (card.isFileOpen()) {
	...
	// 書き換えには時間がかかるので画面にメッセージを表示する
    tft.queue.reset();
    tft_string.set("WiFi firmware updating...");
  	tft.canvas(0, 0, TFT_WIDTH, TFT_HEIGHT);
  	tft.set_background(COLOR_BACKGROUND);
  	tft.add_text(tft_string.center(TFT_WIDTH), TFT_HEIGHT / 2,
  										COLOR_STATUS_MESSAGE, tft_string);
...
///    clear_cur_ui();
    hal.watchdog_refresh();
    ui.clear_lcd();		// 画面消去
    ui.reset_status();	// ステータスにファイル名が表示されるのを消去
  }

  // WiFiのSSIDとパスワードをファイルから読み取って設定する
  mks_wifi_setting();
}
/// WiFiのSSIDとパスワードをファイルから読み取って設定する
void mks_wifi_setting() {

	hal.watchdog_refresh();

	card.openFileRead((char *)WIFI_NAME_KEY_FILE);	// ファイルオープン
	if (card.isFileOpen())	// ファイルがオープンできたなら
	{
		wifi_delay(2000);
		hal.watchdog_refresh();

		uint32_t siz = card.getFileSize();	// ファイルサイズ
		uint8_t c, *p;

		ZERO(uiCfg.wifi_name);	// 格納先をゼロクリアしておく
		ZERO(uiCfg.wifi_key);

		p = uiCfg.wifi_name;	// SSID名称格納先の先頭
		for (uint32_t i = 0; i < siz; i++)
		{
			card.read(&c, 1);	// 1文字読み取り
			if (c == '\n' || c == '\r')	// 行末か？
			{
				p = uiCfg.wifi_key;	// パスワード格納先の先頭
			}
			else
			{
				*p++ = c;	// 文字を格納
		...
		card.closefile();	// ファイルを閉じる
	
		uiCfg.configWifi = true;	// WiFiコンフィグリクエスト
		cfg_cloud_flag = 1;			// クラウド設定(初期値のままディセイブル)

		hal.watchdog_refresh();

		// ファイルをリネームする
		card.removeFile((char *)WIFI_NAME_KEY_FILE_RENAME);
		MediaFile file, *curDir;
		const char * const fname =
				card.diveToFile(false, curDir, WIFI_NAME_KEY_FILE);
		if (file.open(curDir, fname, O_READ))
		{
			file.rename(curDir, (char *)WIFI_NAME_KEY_FILE_RENAME);
			file.close();
		}
		ui.reset_status();
	}
...
void mks_wifi_init()
{
	...
	/// tft_lvgl_configuration.cppのtft_lvgl_init()を移植
	hal.watchdog_refresh();
	mks_esp_wifi_init();
	hal.watchdog_refresh();
