...
void MarlinUI::tft_idle() {
    ...
    if (draw_menu_navigation) {
	  ...
	  #if 1	/// KP3追加(ボタンの位置を左右にずらし周囲に枠を付ける)
	  tft.canvas(0, 205, 320, 34);
	  tft.set_background(COLOR_BLACK);
	  uint16_t col;
	  col = encoderTopLine > 0 ? COLOR_WHITE : COLOR_DARKGREY;
	  tft.add_rectangle(0, 0, 34, 34, col);
	  col = encoderTopLine + LCD_HEIGHT < screen_items ?
	  							COLOR_WHITE : COLOR_DARKGREY;
	  tft.add_rectangle(286, 0, 34, 34, col);
	  tft.add_rectangle(143, 0, 34, 34, COLOR_WHITE);

      add_control(1, 206, PAGE_UP, imgPageUp, encoderTopLine > 0);
      add_control(287, 206, PAGE_DOWN, imgPageDown, encoderTopLine + LCD_HEIGHT < screen_items);
...
void draw_fan_status(uint16_t x, uint16_t y, const bool blink) {
  ...
  tft.add_image(0, 10, image, col);

  tft_string.set(ui8tostr4pctrj(thermalManager.fan_speed[0]));
  tft_string.trim();
  tft.add_text(tft_string.center(64) + 6, 72, col, tft_string);
  #else
  if (fanSpeed >= 127)
    image = blink ? imgFanFast1 : imgFanFast0;
  else if (fanSpeed > 0)
...
void MarlinUI::draw_status_screen() {
  ...
  // print duration
  #if 1	/// KP3変更(進捗パーセントと推定残り時間を追加)
  char buffer[56];
  duration_t elapsed = print_job_timer.duration();
  elapsed.toDigital(buffer);
  strcat(buffer, "  ");
  strcat(buffer, ui8tostr3rj(ui.get_progress_percent()));
  strcat(buffer, "%  ");
  duration_t remaining = get_remaining_time();
  remaining.toDigital(buffer + strlen(buffer));

  tft.canvas(70, 176, 180, 20);
  tft.set_background(COLOR_BACKGROUND);
  tft_string.set(buffer);
  tft.add_text(tft_string.center(180), 0, COLOR_PRINT_TIME, tft_string);
  #else
  char buffer[14];
  duration_t elapsed = print_job_timer.duration();
  elapsed.toDigital(buffer);
