...
/// KP3追加
void store_settings_yes();
void load_settings_yes();
void reset_settings_yes();
...
#if 1 /// KP3変更(EEP操作では1発で確定させずにいったん確認画面を出す)
  #if ENABLED(EEPROM_SETTINGS)
    YESNO_ITEM(MSG_STORE_EEPROM, store_settings_yes, nullptr,
            GET_TEXT_F(MSG_STORE_EEPROM), (const char *)nullptr, F("?"));
    if (!busy)
      YESNO_ITEM(MSG_LOAD_EEPROM, load_settings_yes, nullptr,
            GET_TEXT_F(MSG_LOAD_EEPROM), (const char *)nullptr, F("?"));
  #endif

  if (!busy)
    YESNO_ITEM(MSG_RESTORE_DEFAULTS, reset_settings_yes, nullptr,
